    ...
    <div class="value">
      <div class="flexbox space-8">
        <div>
          ...
          <span v-show="loadingCountries"><i class="fas fa-spinner fa-spin"></i></span>
        </div>
        <div>
          <div class="wa-select">
          ...
          <span v-show="loadingRegions"><i class="fas fa-spinner fa-spin"></i></span>
        </div>
      </div>
      <p class="hint">{{ l10n('Доставка будет ограничена только выбранной страной или регионом страны') }}</p>
    </div>
  </WaField>
</template>
...
onMounted(async () => {
  if (setting.country) await loadRegions()
  await loadCountries()
})
